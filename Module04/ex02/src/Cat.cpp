...
/*   Updated: 2025/07/18 12:21:35 by umeneses         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Cat.hpp"

Cat::Cat(void) : AAnimal(), _className("Cat")
{
	_myBrain = new Brain();
	_printer.constructorTitlePrinter(_className, "Weird Cat");
}

Cat::Cat(std::string type) : AAnimal(type), _className("Cat")
{
	_myBrain = new Brain();
...
Cat::Cat(Cat const &src) : AAnimal(src._type), _className(src._className)
{
